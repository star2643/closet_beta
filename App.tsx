...
export default function App(): React.ReactNode {
  ...
  function findMaxValuesInEachRow(twoDArray) {
    const maxValues = [];
  ...
  function findTopThreeMaxValuesWithIndicesOptimized(maxArray) {
    ...
    launchImageLibrary({ mediaType: 'photo', maxHeight: 640, maxWidth: 640 }, async response => {
      if (!response.didCancel) {
        if (response.assets && response.assets.length > 0) {
          ...
          setResult(topThreeLabels.toString())
          const numDetections = 8400; // 总检测数
          const Alldetections = [];
          // for (let i = 0; i < numDetections; i++) {
          //   const x = outputTensor[i + 8400 * 43];
          //   const y = outputTensor[i + 8400 * 44];
          //   const width = outputTensor[i + 8400 * 45];
          //   const height = outputTensor[i + 4725 * 46];
